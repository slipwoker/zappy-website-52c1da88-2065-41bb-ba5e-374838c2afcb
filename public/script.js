document.addEventListener('DOMContentLoaded', function () {
  ...
  if (navbar) {
    ...
    // Run on init in case page is already scrolled
    handleNavbarScroll();
  }
  ...
  if (contactForm) {
    ...
    function validateField(field) {
      ...
      if (['submit', 'button', 'reset', 'hidden', 'file'].includes(type)) return true;

      // Required check
      if (isRequired && !VALIDATORS.required(value)) {
        showError(field, field.dataset.errorRequired || ERROR_MESSAGES.required);
        return false;
      }
      ...
      if (type === 'email' && !VALIDATORS.email(value)) {
        showError(field, field.dataset.errorEmail || ERROR_MESSAGES.email);
        return false;
      }

      // Phone check
      if (type === 'tel' && value.trim() !== '' && !VALIDATORS.phone(value)) {
        showError(field, field.dataset.errorPhone || ERROR_MESSAGES.phone);
        return false;
      }

      // Min length check
      if (minLength && !VALIDATORS.minLength(value, minLength)) {
        showError(field, field.dataset.errorMinlength || ERROR_MESSAGES.minLength);
        return false;
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "פרוטאין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '52c1da88-2065-41bb-ba5e-374838c2afcb',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '52c1da88-2065-41bb-ba5e-374838c2afcb';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '52c1da88-2065-41bb-ba5e-374838c2afcb';
